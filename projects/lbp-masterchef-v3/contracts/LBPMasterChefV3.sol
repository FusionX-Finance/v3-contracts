...
contract LBPMasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    /// @notice Address of RFUSIONX contract.
    IERC20 public immutable RFUSIONX;
    ...
    /// @param _RFUSIONX The Reward FUSIONX token contract address.
    /// @param _nonfungiblePositionManager the NFT position manager contract address.
    constructor(IERC20 _RFUSIONX, INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) {
        RFUSIONX = _RFUSIONX;
    ...
    /// @notice View function for checking pending RFUSIONX rewards.
    ...
    function setReceiver(address _receiver) external onlyOwner {
        ...
        if (RFUSIONX.allowance(_receiver, address(this)) != type(uint256).max) revert();
        receiver = _receiver;
    ...
    /// @notice Update the given pool's RFUSIONX allocation point. Can only be called by the owner.
    ...
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyReceiver {
        ...
        RFUSIONX.safeTransferFrom(receiver, address(this), _amount);
    ...
    /// @notice Safe Transfer RFUSIONX.
    /// @param _to The RFUSIONX receiver address.
    /// @param _amount Transfer RFUSIONX amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            ...
            RFUSIONX.safeTransfer(_to, _amount);
        }
    }
